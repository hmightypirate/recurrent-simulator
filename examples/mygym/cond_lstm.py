...
class LSTMAction(LSTMBase):
    ...
    def __init__(self, in_size, n_actions, out_size=None, **kwargs):
        
        if out_size is None:
            in_size, out_size = None, in_size
        super(LSTMAction, self).__init__(in_size, out_size, **kwargs)

        # Linear tx
        self.affine_tx = linear.Linear(n_actions, out_size)
        
        self.reset_state()

    def to_cpu(self):
        super(LSTMAction, self).to_cpu()
    ...
    def to_gpu(self, device=None):
        super(LSTMAction, self).to_gpu(device)
        if self.c is not None:
    ...
    def __call__(self, x, a):
        ...
        Args:
            ...
            a (~chainer.Variable): batch of actions
        ...
        if self.h is not None:
            ...
            if batch == 0:
                ...
                h_a = self.affine_tx(a)
                h_update = h_update * h_a
