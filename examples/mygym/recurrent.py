...
from chainerrl.agent import ABCMeta
from chainerrl.agent import abstractmethod
import collections
import contextlib

import chainer
from cond_lstm import LSTMAction
...
def get_state(chain):
    ...
    for l in chain.children():
        if isinstance(l, chainer.links.LSTM) or isinstance(l, LSTMAction):
...
def stateful_links(chain):
    for l in chain.children():
        if (isinstance(l, (chainer.links.LSTM, Recurrent)) or
            isinstance(l, (LSTMAction, Recurrent))):
            yield l
        elif isinstance(l, (chainer.Chain, chainer.ChainList)):
